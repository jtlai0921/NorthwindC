...
set @i='10,000.12345'
--select @i 'i',CAST(@i as decimal(11,5)) '貨幣型態'
select @i 'i',convert(money,@i) '貨幣型態'
go
...
set @i=123456
set @j=@i
select @i 'i值',@j 'j值',cast(@j as int) '二進位轉整數',DATALENGTH(@j) 'J值長度'
...
--建立資料表
IF  EXISTS (SELECT name FROM sys.tables WHERE name='Mytable')
DROP table [Mytable]
GO
...
--新增一筆資料
insert MyTable (書籍編號,書籍名稱,價格,圖檔) VALUES (101,'Windows使用手冊','5,000',convert(varbinary(100),123456789))
go
insert MyTable (書籍編號,書籍名稱,價格,圖檔) VALUES (102,'Windows使用手冊','5,000',convert(varbinary(100),'臺北市職能發展學院'))
go

--查詢資料
SELECT 書籍編號,書籍名稱,價格,圖檔 ,convert(int , 圖檔) '圖檔文字' FROM MyTable where 書籍編號=101
go
SELECT 書籍編號,書籍名稱,價格,圖檔 ,convert(varchar(100) , 圖檔) '圖檔文字' FROM MyTable where 書籍編號=102
go
...
DECLARE @VarTable TABLE (
   [COL1] [int] IDENTITY(1,1) NOT NULL,
   [COL2] [nvarchar](50) Default'無',
   [COL3] [sql_variant] )

INSERT INTO @VarTable (COL2,COL3) VALUES ('蘋果','資料存檔')
INSERT INTO @VarTable (COL2,COL3) VALUES ('鳳梨','100.33')
INSERT INTO @VarTable (COL2,COL3) VALUES ('蓮霧','12/12/2007 13:13:13.000')
INSERT INTO @VarTable (COL2,COL3) VALUES ('西瓜',CAST('12/12/2007 13:13:13.000' AS DATETIME))
INSERT INTO @VarTable (COL3) VALUES (NULL)
